...
{
	NSDateComponents *components = [CURRENT_CALENDAR components:DATE_COMPONENTS fromDate:self];
//	return components.weekday; //默认：
    
    //==>周日 == 星期1  (1--7)外国佬算法
    
    if (components.weekday==1)//星期天（西方为起始）
        return 7;
...
- (NSInteger) age
{
    ...
    NSDateComponents *components_now = [CURRENT_CALENDAR components:DATE_COMPONENTS fromDate:[NSDate date]];
	return components_now.year - components.year;
}
