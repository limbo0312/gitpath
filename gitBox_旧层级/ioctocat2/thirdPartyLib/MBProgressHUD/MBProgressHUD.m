...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.mode = MBProgressHUDMode_EGS_CUSTOM;//MBProgressHUDModeIndeterminate;
		self.labelText = nil;
...
- (void)showWhileExecuting:(SEL)method onTarget:(id)target withObject:(id)object animated:(BOOL)animated {
	...
	targetForExecution = target;//MB_RETAIN(target);
	objectForExecution = object;//MB_RETAIN(object);
...
//        MB_AUTORELEASE([[UIActivityIndicatorView alloc]
//										 initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge]);
		[(UIActivityIndicatorView *)indicator startAnimating];
		[self addSubview:indicator];
	}
    else if (mode == MBProgressHUDMode_EGS_CUSTOM )
    {
        //EGS custom img
        [indicator removeFromSuperview];
...
//            MB_AUTORELEASE([[MBRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
...
//        CGContextSetFillColorWithColor(context, [UIColor whiteColor].CGColor);
        CGFloat fillColor[4]    = {1.0,1.0,1.0,0.9};
...
static const char* mbProgressHUDKey = "MBProgressHUD";
...
//- (MBProgressHUD *)mbProgressHUD{
//    id result = [self mbProgressHUD_core];
//    return result;
//}

- (void)setMbProgressHUD:(MBProgressHUD *)mbProgressHud{
    objc_setAssociatedObject(self, mbProgressHUDKey, mbProgressHud, OBJC_ASSOCIATION_ASSIGN);
}
...
- (void)showProgressHUD{
    ...
    if (self.mbProgressHUD==nil) {
        [self setMbProgressHUD:[MBProgressHUD showHUDAddedTo:self animated:YES]];
    }
...
- (void)hideProgressHUD{
    ...
    @catch (NSException *exception) {
        DebugLog(@"MBProgressHUD exception is %@  ==>多为重复隐藏",exception);
    }
    @finally {
...
- (void)showProgressHUDWithText:(NSString *)text delay:(float)delay{
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self animated:YES];
